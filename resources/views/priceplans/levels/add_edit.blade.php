                    ...
                    <div class="card card-preview">
                        <div class="card-inner">
                            <div class="preview-block">
                                <span class="preview-title-lg overline-title">@if(isset($model) && $model->id) Edit @else Add @endif Price plan level</span>
                                @if(isset($model))
                                <form action="{{ route('priceplan.levels.update',['level'=>$model->id]) }}" method="post">
                                    @method('PUT')
                                    @else
                                    <form action="{{ route('priceplan.levels.store') }}" method="post">
                                        @endif
                                        @csrf
                                        {{-- <div class="row">
                                        ...
                                        <div class="row mt-4">
                                            ...
                                            <div class="col-lg-4">
                                                ...
                                                <a href="{{ route('priceplan.levels.index') }}" class="btn btn-success"> Back</a>
                                            </div>
                                        </div>
                                    </form>
                                </div>
